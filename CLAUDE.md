...
새 기능 추가 요청을 받으면, 코드 작성 전에 반드시 다음 3개 문서를 먼저 생성할 것.

- `[기능명]-spec.md` — PRD/요구사항 (사용자 스토리, 입력/출력, 제약, 데이터 모델)
- `[기능명]-plan.md` — phase 단위 개발 계획 (기술 스택, phase별 목표, 완료 기준)
- `[기능명]-tasks.md` — 체크박스 기반 세부 작업 리스트 (phase별로 그룹화)

기존 회원가입/로그인 기능의 `spec.md` / `plan.md` / `tasks.md` 3종 구조를 동일하게 적용한다.
파일명 prefix는 기능을 식별할 수 있도록 영문 또는 한글로 명명할 것 (예: `review-spec.md`, `review-plan.md`, `review-tasks.md`).
